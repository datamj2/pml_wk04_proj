...
The original training and test sets were obtained from ******. The structure of the first 10 columns is shown below.
```{r}
...
ind<-ps2==ts2$classe
ind2<-ps2!=ts2$classe
outsample_err<-100*(sum(ind2)/length(ts2$class)) # in sample error
outsample_acc<-100*(sum(ind)/length(ts2$class)) # in sample accuracy
```
