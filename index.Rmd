...
tr2<-tr2[,-c(1:6)]
cat(sprintf('Training set dimensions after feature selection, rows x cols: %i x %i ',dim(ts2)[1],dim(ts2)[2]))
```
Note that only the partioned training set will be reduced in (column) size: this step is not necessary for the partioned test set.
